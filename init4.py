...
training_data = Coach.gen_tr_data(puzzle, episodes=300, depth=5)
...
for e in range(EPISODES):
    ...
    for t in range(EPISODE_SIZE):  # max step count within a single episode
        move_counter += 1
        if move_counter % agent.memory_size == 0:
            training_data = Coach.gen_tr_data(puzzle, episodes=300, depth=5)
            agent.tr_data = training_data
        ...
        if was_random:
            ...
            thought_moves += 1
        puzzle.move(action)
        # print(f'action taken: {action}  ==> {puzzle.get_last_move_name()}')
